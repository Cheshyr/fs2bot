                        ...
                        if (doc3.players.includes(message.author.id)) {
                            ...
                            players.findOne({ discordID: message.author.id }).then((doc2) => {
                                ...
                                if (doc2 === null) {
                                    ...
                                    events.findOneAndUpdate({ tID: a }, { $pull: { players: message.author.id } }).then((updatedDoc) => {
                                        console.log(updatedDoc);
                                        return message.author.send(message.author.username + ` (` + doc2.FS2_username + `) withdrawn from tournament ` + updatedDoc.tID);
                                    })
                                }
                            }) 
                        }
                        else {
                            return message.author.send(`you are not signed up for tournament ` + a);
                        }
                    })
